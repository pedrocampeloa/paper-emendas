...
def run_plr_main(df: pd.DataFrame, controls: list = None,
    ...
    controls = [c for c in controls if c not in (target, treatment, cluster_col)]
    ...
    cols = [target, treatment] + controls + [cluster_col]
    seen, cu = set(), []
    for c in cols:
    ...
    if len(work) < 1000:
        return None
    if work[cluster_col].nunique() < 30:
        return None

    # Within-deputy demeaning
    cols_dem = [target, treatment] + controls
    work = within_transform(work, cluster_col, cols_dem)
    ...
    df_dml = pd.concat([work[[target]], T_s, X_s, work[[cluster_col]]], axis=1)
    data = DoubleMLClusterData(df_dml, y_col=target, d_cols=treatment,
                                  cluster_cols=cluster_col, x_cols=list(controls))
...
def run_pliv_main(df: pd.DataFrame, controls: list = None,
    ...
    controls = [c for c in controls
                  if c not in (target, treatment, cluster_col)
                  and c not in ivs]
    controls = [c for c in controls if c in df.columns
                  and df[c].notna().mean() > 0.5
                  and df[c].nunique() > 1]
    ...
    cols = [target, treatment] + controls + ivs + [cluster_col]
    seen, cu = set(), []
    for c in cols:
        if c not in seen: cu.append(c); seen.add(c)
    work = df[cu].dropna().copy()
