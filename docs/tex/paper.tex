...
\midrule
First-stage $F$                      & 4{,}214           & 4{,}096           & 10{,}275          \\
Anderson--Rubin 95\% CI              & $[-3.74,\,-1.90]$ & $[-0.31,\,+3.95]$ & $[-1.36,\,-0.51]$ \\
Cinelli--Hazlett $\mathrm{RV}_{q=1}$  & 0.366             & 0.450             & 0.601             \\
$N$                                  & 869{,}902          & 226{,}308          & 643{,}594          \\
Clusters (deputies)                  & 931               & 614               & 597               \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
This section reports the two coefficients that organize the empirical evidence. Subsection~\ref{subsec:main_gov} reports the canonical pork-for-votes effect on Executive alignment and documents the sign reversal between the 55th and 56th Legislatures. Subsection~\ref{subsec:legislative_capture} reports the analogous coefficient on alignment with the party of the Chamber president and documents that the bargain re-emerges in the institutional space controlled by Arthur Lira after February 2021. Section~\ref{sec:polarization} describes the polarization environment of the sample window; Section~\ref{sec:discussion} discusses the institutional-transfer interpretation.
...
Pooled (reference)         & $-2.82^{***}$ & $[-3.74,\,-1.90]$ & 869{,}902 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: PLIV-DML with full-clean controls plus party fixed effects, within-deputy demeaning, deputy-clustered standard errors, backlog instrument, 60-day pre-vote window. The pooled row is reported for reference and is not interpreted as a structural average because it aggregates two heterogeneous regimes. Stars: $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}

The two legislature-specific coefficients describe a clear sign reversal. Under Temer, an additional one million reais of pre-vote committed amendment raises the probability of Executive alignment by $1.73$ percentage points, statistically significant at the five-percent level and consistent in magnitude with the classical \citet{PereiraAndMueller2004} estimate on pre-2015 data. Under Bolsonaro, the same instrument predicts a $0.94$-point decrease in Executive alignment, statistically significant at the one-percent level. The Anderson--Rubin interval for the Bolsonaro coefficient is essentially identical to the cluster-robust interval; the Temer-era AR interval is wider and marginally includes zero, reflecting the smaller sample (Section~\ref{sec:strategy}). A fully non-parametric robustness check based on the Causal IV Forest of \citet{AtheyTibshiraniWager2019} reproduces the sign reversal: the median pointwise CATE is $+1.79$ percentage points in Legislature 55 and $-1.97$ in Legislature 56, essentially identical to the parametric benchmark (Appendix~\ref{app:cforest}, Table~\ref{tab:cforest}).
...
% ============================================================================
\section{Polarization and institutional context}
\label{sec:polarization}
% ============================================================================

The regime change documented in Section~\ref{sec:results} coincides with a marked shift in the polarization environment of the Brazilian Chamber. This section characterizes that shift descriptively as institutional context for the results above.

Standard spatial models of polarization code abstentions as intermediate positions and apply continuous distance metrics to roll-call vectors. In Brazilian roll-call data this convention has a known empirical problem: abstention and procedural obstruction are widely used strategic maneuvers rather than signals of intermediate ideology, and the continuous Euclidean metric artificially smooths conflict precisely when tactical maneuvering is most intense. To address this, we borrow two complementary measures developed in a companion working paper \citep{AlbuquerqueSousaCajueiroMueller2026WP}. The MDS-Euclidean measure is the classical continuous distance computed on bimestrial roll-call matrices via multidimensional scaling. The between-bloc \emph{antagonism} measure is the rate of direct opposition between deputies conditioned on jointly substantive votes, capturing hard, expressed ideological confrontation while explicitly treating obstruction as opposition rather than as a neutral midpoint. The companion paper validates the antagonism measure against external benchmarks (correlation $r = 0.93$ with independent expert-survey placements of party ideology; agreement of $89\%$ with the formal government-opposition division of the Chamber) and shows that the major Brazilian polarization episodes are episodes of \emph{disciplined antagonism between cohesive blocs} rather than of intra-bloc fragmentation.

Figure~\ref{fig:pol_traj} reports the time series of the two measures from 2014 through 2024, normalized to z-scores. Panel (A) overlays the trajectory with the calendar of Brazilian presidents; panel (B) does the same with the calendar of Chamber-of-Deputies presidents. Two patterns are noteworthy. First, both measures peak in the Bolsonaro presidency (2019--2022), with antagonism reaching its sample maximum in 2021. Second, the trajectories under Maia (DEM, July 2016 through January 2021) and under Lira (PP, February 2021 onward) are visibly different: Maia's tenure starts at low polarization (Temer era) and crosses the institutional inflection of 2018--2019; Lira's tenure starts at the post-2018 peak and trends downward through 2024. The Chamber-president panel shows that the post-Lira trajectory is one of gradual de-polarization, consistent with the institutional transfer of the bargaining instrument from the Executive to the Chamber leadership coinciding with a moderation of legislative confrontation once the new equilibrium is established.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_polarization_traj_B.pdf}
\caption{Brazilian legislative polarization 2014--2024 across institutional sub-periods. Both measures are normalized to z-scores. Panel (A) shades Brazilian presidential sub-periods (Rousseff, Temer, Bolsonaro, Lula~III); panel (B) shades Chamber-of-Deputies presidential sub-periods (Cunha, Maia, Lira, Motta). Both measures peak in the Bolsonaro period, with antagonism reaching its sample maximum in 2021. The Lira sub-period coincides with a gradual de-polarization across both measures.}
\label{fig:pol_traj}
\end{figure}
...
Two limitations bound the interpretation. The parallel Secret Budget channel introduced an opaque dimension of executive-legislative bargaining during 2020--2022 that we cannot measure at the deputy level, so the Bolsonaro-era coefficient on the Executive outcome captures the effect of visible amendments only; the post-2022 RP-8 and Pix substitution documented in Subsection~\ref{subsec:rp9_episode} shows that the legislative-capture channel persists beyond our sample window through different institutional vehicles. The Temer-era estimate is also identified off inter-annual variation that the backlog instrument deliberately exploits; adding year fixed effects absorbs this variation and attenuates the estimate, as we document in Appendix~\ref{app:robustness}. Neither limitation overturns the central finding. A natural and promising extension is to bring the rhetorical channel into the same framework. Floor-speech sentiment can serve as an alternative treatment in the DML setup and would enable a direct horse race between pork and rhetoric as instruments of executive influence. A second extension is the allocative side of the bargain: the function-by-function and region-by-region distribution of amendment commitments would shed light on how the institutional regime shapes the targeting of distributive resources beyond their effect on alignment. We leave both for future work.
...
Table~\ref{tab:ablation} reports the sociodemographic ablation summarized in Section~\ref{sec:strategy}. The Bolsonaro-era estimate is essentially invariant across the six specifications; the Temer-era estimate is more sensitive but the qualitative reversal between legislatures is preserved.
...
The mean and the median of the Winsorized CATE distribution differ substantially in absolute magnitude. This is expected: the forest's local IV moment equation is sensitive to the leaf-level instrument strength, and pointwise CATE estimates in leaves with weaker instrumental variation inflate the variance of the distribution. The median is the appropriate central-tendency statistic in this setting, and it is the median that reproduces the parametric benchmark. We treat the forest exercise as a robustness reading that corroborates the narrative of Section~\ref{sec:results} rather than as a parallel main estimate; the PLIV-DML specification, with its $\sqrt{n}$-convergent ATE under cross-fitted ElasticNet \citep{Chernozhukov2018}, remains the preferred econometric framework.
